...
import { AuroraCard, AuroraCardContent, AuroraBox, AuroraTypography, AuroraTable, AuroraTableBody, AuroraTableCell, AuroraTableContainer, AuroraTableHead, AuroraTableRow, AuroraChip, AuroraLiveIconActivity, AuroraLiveIconBadgeAlert } from '@acentra/aurora-design-system';
...
export function PipelineAnalyticsWidget() {
  ...
  const loadData = async () => {
    try {
      ...
      const stageMap = new Map<string, { count: number; totalDays: number; candidates: any[] }>();
      ...
      candidates.forEach((candidate: any) => {
        if (candidate.pipeline_status_id && stageMap.has(candidate.pipeline_status_id)) {
      ...
      const stageData: PipelineStageData[] = statuses.map((status: any) => {
        const stage = stageMap.get(status.id)!;
        const avgDays = stage.count > 0 ? Math.round(stage.totalDays / stage.count) : 0;
          ...
          <AuroraCardContent sx={{ p: 0 }}>
            ...
            <AuroraBox sx={{ p: 2 }}>
              {bottlenecks.length === 0 ? (
                <AuroraTypography variant="body2" color="text.secondary" sx={{ textAlign: 'center', py: 2 }}>
                  No bottlenecks detected
                </AuroraTypography>
              ) : (
                bottlenecks.map((stage, index) => (
                  <AuroraBox key={stage.id} sx={{ mb: index < bottlenecks.length - 1 ? 2 : 0 }}>
                    <AuroraBox sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                      <AuroraTypography variant="body2" sx={{ fontWeight: 500 }}>
                        {stage.label}
                      </AuroraTypography>
                      <AuroraChip
