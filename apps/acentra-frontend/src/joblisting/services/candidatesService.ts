...
export const candidatesService = {
  async getCandidates(page: number = 1, limit: number = 25): Promise<CandidatesResponse> {
  ...
  async createCandidate(data: CreateCandidateData): Promise<Candidate> {
    ...
    if (data.current_address) formData.append('current_address', data.current_address);
    ...
    if (data.desired_salary) formData.append('desired_salary', data.desired_salary.toString());
  ...
  async addCandidateComment(id: string, text: string, attachment?: File): Promise<any> {
    const formData = new FormData();
    ...
    const response = await apiClient.post(`/candidates/${id}/comments`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
