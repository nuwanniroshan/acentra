...
export class S3FileUploadService implements IFileUploadService {
  ...
  constructor(config?: Partial<FileUploadConfig>) {
    this.config = {
      ...
      s3BucketName:
        config?.s3BucketName || process.env.S3_BUCKET_NAME || "acentra_storage",
      maxSizeBytes:
        config?.maxSizeBytes ||
  ...
  async upload(input: UploadInput, path: string): Promise<UploadResult> {
    // Validation
  ...
  private validateFile(input: UploadInput) {
      // Validate Type
